# OpenClaw：main agent 回答時附上 mp3 語音

目標：main 這隻 agent 每次回答，除了原本的文字，**再附一段 mp3 語音**一起送出。

OpenClaw 本身就有這個機制（Auto-TTS），不用寫外掛，只要把 `tts` 打開並指定輸出格式。
要動的是兩個地方：
...
分兩層是刻意的：金鑰和格式放共用層，其他 agent 不會被一起打開。
per-agent 的區塊會 deep-merge 蓋在共用層上，後寫的贏。
...
```bash
php tools/openclaw_apply_tts.php --dry-run   # 先看合併後長什麼樣，不寫檔
php tools/openclaw_apply_tts.php             # 確認沒問題再寫入（會自動備份）
```

預設改的是 `~/.openclaw/openclaw.json`，只碰 `tts` 和 `agents.entries.main.tts`
兩條路徑，`channels`、`models`、`bindings` 原封不動。寫入前會先存一份
...
--agent=support                   # 要開語音的 agent id（預設 main）
...
把 [`tts-main.json5`](./tts-main.json5) 裡的兩個區塊貼進 `~/.openclaw/openclaw.json`。
...
- per-agent 覆寫：<https://docs.openclaw.ai/gateway/config-agents/entries-and-multi-agent>
