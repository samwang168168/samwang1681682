...
在本專案：`MessengerChannel::challenge()` 做掉了。它只在 `hub.mode` 存在時回非 null，
`WebhookHandler` 收到非 null 就直接回傳、不進後續流程。

❌ 常見錯誤：回 `{"challenge":"1158201444"}`、回 `"1158201444"`（帶引號）、
把整個回應包成 JSON。三種都會驗證失敗 —— 必須是原樣的純文字。
...
比對時用定值時間比較（PHP 的 `hash_equals`），不要用 `===`。
實作見 `src/Channel/MessengerChannel::verify()`。

★ 一定要對**原始請求 body 的位元組**算。先 `json_decode` 再 `json_encode` 回去的字串
算出來的 HMAC 對不上，而且症狀是「有中文就壞、純英文就好」這種時好時壞，最難查。
`WebhookRequest` 保留 `rawBody` 就是為了這件事。
