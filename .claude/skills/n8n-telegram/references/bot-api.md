...
curl -X POST "https://api.telegram.org/bot$TOKEN/setWebhook" \
  -d "url=https://n8n.example.com/webhook/telegram" \
...
- `secret_token` —— Telegram 會在每個請求帶 `X-Telegram-Bot-Api-Secret-Token` header,
  用 Webhook node 時**應該驗它**,否則任何人知道你的 URL 就能偽造訊息。
  用 Telegram Trigger 節點的話 n8n 自己管,不用設。
- `drop_pending_updates=true` —— 換 URL 時丟掉積壓的舊訊息。
  不加的話,bot 一上線會被幾百則舊訊息灌爆。
- `max_connections` —— 預設 40,n8n 扛不住就調小。
...
|---|---|---|
| 需要公開 HTTPS | 是 | 否 |
| n8n 支援 | Telegram Trigger / Webhook node | 要自己用 Schedule + HTTP Request 兜 |
| 延遲 | 最低 | 一個 polling 週期 |
